...
import { schoolModel, userModel, userSessionModel, visitorModel } from "../database";
import { apiResponse, loginStatus, SMSTemplate, URL_decode, userStatus } from "../common";
...
import { Re_activation_mail, email_verification_mail, forgot_password_mail, welcome_mail, welcome_mail_google_SL_facebook_SL } from "../helpers/mail";
...
export const signUp = async (req: Request, res: Response) => {
  ...
  try {
    let body = req.body, response: any, otpFlag = 1, authToken = 0, phone_otp = 0;
    ...
    body.password = hashPassword; 2222
    ...
    if (body.email) {
      await email_verification_mail(body, authToken).then(result => { return result }).catch(error => { return error })
    }
    if (body.phoneNumber) {
      await sendSMS(body.countryCode, body.phoneNumber, SMSTemplate?.registration(phone_otp))
    }
...
export const forgot_password = async (req: Request, res: Response) => {
  ...
  let body = req.body,
    ...
    otp = 0, response: any, phone_otp = 0
  try {
    body.isActive = true;
    if (isNaN(body?.email)) {
      ...
      await sendSMS(data?.countryCode, data.phoneNumber, SMSTemplate?.resetPassword(phone_otp))
      await userModel.findOneAndUpdate(body, { phone_otp })
      return res.status(200).json(new apiResponse(200, `OTP has been sent to ${data.phoneNumber}, kindly follow the instructions`, { phone_otp }, {}));
    }
...
export const otp_verification = async (req: Request, res: Response) => {
  reqInfo(req);
  let body = req.body, new_response: any, data: any
  try {
    body.isActive = true;
    ...
    if (!data)
      return res.status(400).json(new apiResponse(400, "Your phone number or email OTP is invalid", {}, {}));
    if (data.isBlock == true)
      return res.status(403).json(new apiResponse(403, "Account has been blocked", {}, {}));
    ...
    if (!data?.isEmailVerified && body?.otp) {
      new_response = await userModel.findOneAndUpdate(body, { isEmailVerified: true, isVerify: true }, { new: true })
    ...
    if (!data?.isPhoneVerified && body?.phone_otp) {
      new_response = await userModel.findOneAndUpdate({ phone_otp: data?.phone_otp }, { isPhoneVerified: true, isVerify: true }, { new: true });
    ...
    if (data)
      new_response = await userModel.findOneAndUpdate(body, { isEmailVerified: true, isPhoneVerified: true, isVerify: true }, { new: true })
...
export const re_active_otp_verification = async (req: Request, res: Response) => {
  reqInfo(req);
  let body = req.body, new_response: any, data: any
  try {
    body.isActive = true;
...
export const resend_otp = async (req: Request, res: Response) => {
  ...
  try {
    if (isNaN(email)) {
      ...
      let data = await userModel.findOne({ phoneNumber: email, isActive: true });
      if (!data)
        return res
          .status(400)
      ...
      if (data.isPhoneVerified == false) {
        ...
        await sendSMS(data?.countryCode, data.phoneNumber, SMSTemplate?.registration(phone_otp))
      } else {
        await sendSMS(data?.countryCode, data.phoneNumber, SMSTemplate?.resetPassword(phone_otp))
      }
      //console.log(response);
      ...
      if (data) {
        await userModel.findOneAndUpdate(
          { phoneNumber: parseInt(email), isActive: true }, { phone_otp: phone_otp, });
...
export const student_teacher_login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isNaN(body?.email))
      ...
      response = await userModel.findOne({ phoneNumber: body.email, isActive: true, $or: [{ userType: userStatus.teacher }, { userType: userStatus.student }, { userType: userStatus.faculty }] }).select("-__v -createdAt -updatedAt");
    if (response) {
      ...
      if (response?.isPremium === false || !response?.isVerify) {
        return res
          .status(401)
          .json(new apiResponse(401, "checkout subscription page", response, {}))
      }
    ...
    if (!response) return res.status(400).json(new apiResponse(400, "Login details are incorrect!", {}, {}));
